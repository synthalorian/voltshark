//! Voltshark firmware entry point (STM32F4).
//!
//! The hardware glue only exists for ARM Cortex-M targets. Pure synthesis
//! logic lives in the `voltshark` library crate and is host-testable.
...
mod firmware {
    ...
    use crate::audio::i2s::I2SAudioEngine;
    use voltshark::midi::parser::MidiEvent;
    use voltshark::synth::engine::SynthEngine;
    ...
    fn main() -> ! {
        ...
        let mut midi_parser = voltshark::midi::parser::MidiParser::new(midi_serial);
...
/// Host stub: the firmware requires ARM Cortex-M hardware. On the host, the
/// `voltshark` library crate (MIDI parser, DSP, voice engine) is what builds
/// and gets tested via `cargo test`.
